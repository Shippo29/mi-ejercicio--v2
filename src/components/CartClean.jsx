...
export default function CartClean() {
  const {
    carrito = [],
    incrementar,
    decrementar,
  ...
  const visibleItems = carrito.filter((it) => it && it.id !== 101);
  const couponIsError =
    couponMessage && /no|invál|no se/i.test(String(couponMessage));
    ...
    <div className="cart-page container">
          ...
          {carrito.some((it) => it && it.id === 101) && (
            <div className="test-product-note">
              <div>
                ...
                prueba y no se muestra en el listado principal.
              </div>
              <div>
                <button
      ...
      <div className="coupon-row">
        <div className="coupon-input d-flex" style={{ gap: 12 }}>
          ...
          {activeCoupon ? (
            <button className="btn-danger-soft" onClick={handleRemove}>
              Quitar
            </button>
          ) : (
